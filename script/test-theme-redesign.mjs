...
import { families, displayFaces, skinFiles, retainedStyles, repo, hash, within, freshOutput, RPN, readRegistry, loadThemes, selectThemes, checkProvenance, checkSkin, themeManifest } from './test-theme-redesign-static.mjs';
...
const retainedSystemFonts = new Set(['Georgia', 'ui-monospace']);
const nativeAliases = { 'Theme Unbounded': 'Unbounded', 'Theme Doto': 'Doto', 'Theme Computer Modern': 'cmr10', 'Theme Euler Fraktur': 'eufm10', 'Theme Climate': 'Climate Crisis', 'Theme Fascinate': 'Fascinate Inline', 'Theme Pixelify': 'Pixelify Sans', 'Theme Formal Script': 'rsfs10' };
function usedSelectedFace(row) {
  if (retainedSystemFonts.has(row.family)) return row.fonts.some(f => !f.isCustomFont);
  const expected = normalizeFamily(nativeAliases[row.family] || row.family);
  ...
  try {
    ...
    page.on('request', request => { void (async () => {
      ...
      if (font) {
        ...
        if (failFonts && (entry.relative.includes('/theme-library/') || retainedStyles[job.theme] && entry.relative.startsWith('assets/fonts/'))) { network.failedFonts++; return request.abort('failed'); }
      }
...
async function runCase(job, fallback = false) {
  ...
  try {
    ...
    if (!fallback) {
      ...
      result.displayPolicy = retainedStyles[job.theme] ? 'retained optional/native display; warm adoption checked' : 'selected library display swap';
      for (const row of result.coldDisplaySamples) if (!retainedStyles[job.theme] && !usedSelectedFace(row)) result.issues.push(`cold-display-not-rendered:${row.selector}:${row.family}`);
    ...
    if (fallback) {
      ...
      if (result.fonts.some(r => families[job.theme]?.includes(r.family) && !retainedSystemFonts.has(r.family) && usedSelectedFace(r))) result.issues.push('selected-font-rendered-despite-failure-control');
