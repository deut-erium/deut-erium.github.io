...
if len(post_routes) != 82: fail(f"source post count drift: {len(post_routes)}")
...
if (forms, challenge_scripts, article_scripts, code_frames, math_expressions, images) != (10, 6, 82, 334, 243, 84):
...
if len(math_pages) != 6: fail(f"math page count drift: {len(math_pages)}")
if GOATCOUNTER:
...
if len(archive_order) != 82 or len(set(archive_order)) != 82 or set(archive_order) != {f"/{rel}" for rel in post_routes}:
...
if tag_count != 146 or "146 merged tags" not in archive_text: fail(f"merged tag count drift: {tag_count}")
