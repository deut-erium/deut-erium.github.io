...
export let families, displayFaces, skinFiles, retainedStyles, libraryFamilies;
let loadedConfig;
const retainedIds = new Set(['grid-meltdown', 'the-exploit-grimoire', 'cryptographic-blockbuster', 'exploit-in-b-flat']);
export function readRegistry(root = repo) {
  ...
  assert.equal(ids.length, 48, 'Registry must contain 47 alternatives plus RPN');
...
export function validateThemes(document, registry, files) {
  ...
  for (const [id, row] of Object.entries(document)) {
    ...
    if (row.retained) {
      assert.ok(retainedIds.has(id), `Unexpected retained style: ${id}`);
      const r = row.retained;
      assert.equal(r.source, 'older-selected-style');
      assert.match(r.sha256, /^[a-f0-9]{64}$/);
      assert.match(r.globalCssSha256, /^[a-f0-9]{64}$/);
      assert.ok(Array.isArray(r.families) && r.families.length >= 2 && r.families.every(f => typeof f === 'string' && /^[A-Za-z0-9][A-Za-z0-9 .-]*$/.test(f)));
      assert.equal(new Set(r.families).size, r.families.length);
      assert.ok(r.families.includes(r.display));
      assert.equal(Object.keys(r.fontFiles).length, 16);
      for (const [file, digest] of Object.entries(r.fontFiles)) {
        assert.match(file, /^assets\/fonts\/[a-z0-9-]+\/[a-z0-9.-]+\.woff2$/);
...
export function loadThemes(root = repo) {
  ...
  loadedConfig = config;
  libraryFamilies = Object.freeze(Object.fromEntries(Object.entries(config).map(([id, row]) => [id, row.families])));
  retainedStyles = Object.freeze(Object.fromEntries(Object.entries(config).filter(([, row]) => row.retained).map(([id, row]) => [id, row.retained])));
  families = Object.freeze(Object.fromEntries(Object.entries(config).map(([id, row]) => [id, row.retained?.families || row.families])));
  displayFaces = Object.freeze(Object.fromEntries(Object.entries(config).map(([id, row]) => [id, row.retained?.display || row.display])));
...
export function checkProvenance(root, selectedFamilies = Object.values(libraryFamilies).flat()) {
  ...
  if (fs.existsSync(path.join(root, themeManifest))) {
    ...
    assert.deepEqual(config, loadedConfig, 'Generated theme metadata differs from source expectations');
  }
  return index;
}
...
export function checkSkin(root, skin, index, cssOverride) {
  ...
  if (retainedStyles[skin]) {
    // These four are intentionally older compositions, not failed redesigns.
    // Exact pins protect their accepted source and native fonts; the shared
    // ownership suite still checks all skin declarations independently.
    const retained = retainedStyles[skin];
    assert.equal(hash(css), retained.sha256, `Retained skin changed: ${skin}`);
    assert.equal(hash(fs.readFileSync(path.join(root, 'assets/css/main.css'))), retained.globalCssSha256, 'Retained global CSS changed');
    for (const [file, digest] of Object.entries(retained.fontFiles)) {
      const full = path.join(root, file);
      assert.equal(fs.realpathSync(full), full, 'Retained font symlink');
      const bytes = fs.readFileSync(full);
      assert.equal(hash(bytes), digest, `Retained font changed: ${file}`);
      assert.equal(bytes.subarray(0, 4).toString(), 'wOF2');
    }
    return { skin, file, retained: true, policy: 'Pinned older style and native fonts; not redesigned-font contracts', families: retained.families, fontFiles: Object.keys(retained.fontFiles).length, sha256: hash(css) };
  }
...
export function checkNoGlobalLibrary(root) {
  ...
  for (const file of files) {
    ...
    if (file.endsWith('.css')) for (const r of walkRules(css)) if (/^@font-face$/i.test(r.prelude)) {
      ...
      assert.ok(!Object.values(libraryFamilies).flat().includes(family), `Selected library face declared globally: ${rel}: ${family}`);
...
async function main() {
  ...
  try {
    ...
    if (index && !opt['negative-control']) {
      const skin = themes.find(id => !retainedStyles[id]) || Object.keys(config).find(id => !retainedStyles[id]), css = fs.readFileSync(path.join(repo, skinFiles[skin]), 'utf8');
      ...
      for (const id of themes.filter(id => retainedStyles[id])) {
        const css = fs.readFileSync(path.join(repo, skinFiles[id]), 'utf8');
        assert.throws(() => checkSkin(repo, id, index, css + '\n/* unexpected retained-source change */'), /Retained skin changed/);
      }
      summary.controls.push('unexpected retained-style changes rejected by exact pins');
    }
