...
This library contains 88 families for 48 theme choices: the original 47 alternatives and The Grand Modulo. themes.json maps every theme ID to its skin filename, family array, display family and existing flag. The flag identifies the five accepted implementations; it does not certify completion of the remaining skins.

PROVENANCE.json records all 828 installed files with public acquisition URLs, UTC access dates, byte lengths and SHA-256 hashes: 652 WOFF2 files, 88 license notices and 88 metadata files. These are installed inventory counts, including the preserved full Latin Rubik Dirt file and its selected wordmark subset, not per-page downloads. The original 130 provenance rows and the original assets are preserved. Keep each family's notice and copyright metadata when copying its files. Binaries are unmodified; metadata and notices use Google Fonts revision 77669fa9a8a89271bce79dfc05fd2529efb1a187. CDN binaries are recorded separately by URL and hash; repository TTF byte parity is not claimed.
...
The Grand Modulo adds Limelight for its wordmark and marquee, with existing Gelasio, Barlow and IBM Plex Mono for reading, labels and code. Limelight was copied without modification from the locally cached research collection after checking its original acquisition receipts. No new network acquisition or global font loading was needed.
