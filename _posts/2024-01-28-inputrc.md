...
Press `v` in command mode to open the current command in your editor. Bash uses `$VISUAL` if set, otherwise `$EDITOR` (shame if it's not set to Vim in your `.bashrc`). Saving and closing the editor executes the edited command.
