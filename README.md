...
The picker keeps all 48 choices from the recorded origin/master release: RPN Garden, 43 deployed alternatives and four selected older styles. Those four are Grid Meltdown, The Exploit Grimoire, Cryptographic Blockbuster and Exploit in B-flat. Grimoire uses Euler Fraktur for display text; its reading font is unchanged. The later collection-wide recoloring and Grand Modulo addition remain off master.
...
## Citations and academic PDFs

Public posts have a native **Cite** panel with selectable BibTeX and a `.bib`
download. Citation files and `publications.json` are generated by Jekyll. Hidden,
unlisted, locked and noindex posts are excluded. Challenge archive dates describe
the event, not the publication date of the archive page; citations label that
uncertainty rather than inventing a date.

Academic PDFs are separate from the browser's Ctrl+P layout. The renderer uses
A4, serif text, page numbers, a canonical HTML link, local fonts and pre-rendered
math. Code can continue across pages. Interactive content uses its static
fallback and a link to the original page.

PDF generation is opt-in while font fidelity is being reviewed. With the cached
local toolchain, build a complete candidate using:

```sh
ACADEMIC_PDFS=1 .toolchain/bin/blog-build agent_out/publications/release
```

With Ruby/Jekyll and Node already configured, use `script/build-release.sh`
instead. The renderer accepts `CHROME_BIN` and explicit `--chrome` / `--puppeteer`
paths when run separately; it never downloads browser dependencies. Default
builds omit PDF links. Enabled release builds fail if rendering fails or any
advertised PDF is missing. Do not deploy an intermediate Jekyll-only build with
PDF links enabled.

A post ending in `/example.html` or `/example/` gets `/example.pdf` and
`/example.bib`. PDF-enabled builds add a compact **PDF** action and alternate
metadata. Ctrl+P remains unchanged by the academic renderer.

Known limits: Chromium PDFs are not byte-reproducible; the HTML/BibTeX build
retains its existing reproducibility gate. The current local fonts lack three
emoji glyphs used in the Maybe Someday code listing. Keep PDF publishing opt-in
until a suitable font is supplied and tested. Physical Paper Pro testing and
full visual review of every PDF page remain outstanding. PDF generation does
not unlock posts or execute article scripts, and blocks external resources.
